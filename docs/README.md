# tranSMART 17.1-HYVE-6.1 Installation

Below are the installation instructions for tranSMART version 17.1-HYVE-6.1. If you already have an older version of tranSMART, follow the [upgrade guide](upgrade.md) instead.
...
```
This should create the file `transmart-server/build/libs/transmart-server-17.1-HYVE-6.1-SNAPSHOT.war`.
Run it in production mode with:
```
java -jar transmart-server/build/libs/transmart-server-17.1-HYVE-6.1-SNAPSHOT.war
```
...
java -jar -Dspring.config.location=/path/to/config.yaml transmart-api-server/build/libs/transmart-server-17.1-HYVE-6.1-SNAPSHOT.war
```
...
Start `Solr`:
```bash
pushd transmart-data-17.1-HYVE-6.1/solr
java -jar start.jar &
popd
```
