...
class ModifiersSpec extends RESTSpec {
    ...
    def "get modifiers"() {
        given: "study TUMOR_NORMAL_SAMPLES is loaded"
        def constraintMap = [type: constraints.StudyConstraint, studyId: Config.TUMOR_NORMAL_SAMPLES_ID]

        when: "I get all observations"
        ObservationsMessageProto responseData = getProtobuf(Config.PATH_OBSERVATIONS, toQuery(constraintMap))
        ...
        (0..<selector.cellCount).each {
            ...
            assert selector.select(it, "modifierDimension", "studyId", 'String').equals(Config.TUMOR_NORMAL_SAMPLES_ID)
        }
