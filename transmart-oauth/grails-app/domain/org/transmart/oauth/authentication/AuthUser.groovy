...
import org.transmart.oauth.OAuthID
...
class AuthUser extends Principal {
    ...
    static hasMany = [authorities: Role, groups: UserGroup, oAuthIDs: OAuthID ]
    ...
    def isAdmin() {
		def bAdmin = false;
		authorities.each { if(it.authority==org.transmart.oauth.authentication.Role.ADMIN_ROLE) bAdmin = true; }
		return bAdmin;
	}
     ...
     */
    def isDseAdmin() {
        authorities.any { it.authority == org.transmart.oauth.authentication.Role.DS_EXPLORER_ROLE }
    }
    ...
    static AuthUser createFederatedUser(String federatedId,
        ...
        ret.addToAuthorities(org.transmart.oauth.authentication.Role.findByAuthority(org.transmart.oauth.authentication.Role.SPECTATOR_ROLE))

        ret
    }

    static void removeAll(org.transmart.oauth.authentication.Role role) {
