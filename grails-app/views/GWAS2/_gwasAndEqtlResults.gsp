<g:if test="${results.gwasResults && results.eqtlResults}">
    ...
    <div style="height: 300px; overflow: auto; border-top: 1px solid black; margin-bottom: 10px">
        <g:render template="/GWAS/tableViewResults" plugin="transmart-gwas" model="[
            ...
            'max': results.gwasResults.max,
            'offset': results.gwasResults.offset,
            'totalCount': results.gwasResults.totalCount,
            'wasRegionFiltered': results.gwasResults.wasRegionFiltered,
            'cutoff': cutoff,
            'sortField': sortField,
            'order': order,
            'search': search
    ...
    <div style="height: 300px; overflow: auto; border-top: 1px solid black; ">
        <g:render template="/GWAS/tableViewResults" plugin="transmart-gwas" model="[
                ...
                'max': results.eqtlResults.max,
                'offset': results.eqtlResults.offset,
                'totalCount': results.eqtlResults.totalCount,
                'wasRegionFiltered': results.eqtlResults.wasRegionFiltered,
                'cutoff': cutoff,
                'sortField': sortField,
                'order': order,
                'search': search
