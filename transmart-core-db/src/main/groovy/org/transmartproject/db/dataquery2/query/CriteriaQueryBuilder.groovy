 ...
 */
@Slf4j
class CriteriaQueryBuilder implements QueryBuilder<DetachedCriteria> {
    ...
    DetachedCriteria builder() {
        new DetachedCriteria(ObservationFact, getAlias('observation_fact'))
    }
    ...
    DetachedCriteria build(TrueConstraint constraint) {
        builder()
    }
     ...
     */
    DetachedCriteria build(ModifierConstraint constraint) {
     ...
     */
    DetachedCriteria build(ValueConstraint constraint) {
     ...
     */
    DetachedCriteria build(FieldConstraint constraint) {
        ...
        switch(constraint.operator) {
            case Operator.EQUALS:
                return builder().build {
                    eq(propertyName, constraint.value)
                }
            case Operator.GREATER_THAN:
                return builder().build {
                    gt(propertyName, constraint.value)
                }
            case Operator.GREATER_THAN_OR_EQUALS:
                return builder().build {
                    ge(propertyName, constraint.value)
                }
            case Operator.LESS_THAN:
                return builder().build {
                    lt(propertyName, constraint.value)
                }
            case Operator.LESS_THAN_OR_EQUALS:
                return builder().build {
                    le(propertyName, constraint.value)
                }
            case Operator.CONTAINS:
                return builder().build {
                    'in'(propertyName, constraint.value)
                }
            case Operator.LIKE:
                return builder().build {
                    like(propertyName, constraint.value)
                }
            default:
     ...
     */
    DetachedCriteria build(TimeConstraint constraint) {
     ...
     */
    DetachedCriteria build(BiomarkerConstraint constraint) {
        throw new NotImplementedException()
    }
     ...
     */
    DetachedCriteria build(PatientSetConstraint constraint) {
     ...
     */
    DetachedCriteria build(Negation constraint) {
        builder().build {
            not {
                build(constraint.arg).criteria.each {
                    add(it)
                }
            }
        }
     ...
     * @return
     */
    DetachedCriteria build(Combination constraint) {
        List<DetachedCriteria> parts = constraint.args.collect {
            build(it)
        }
        switch (constraint.operator) {
            case Operator.AND:
     ...
     */
    DetachedCriteria build(TemporalConstraint constraint) {
        ObservationQuery eventQuery = constraint.eventQuery
        QueryBuilder subQueryBuilder = new CriteriaQueryBuilder(
        ...
        switch (constraint.operator) {
            case Operator.BEFORE:
                return ObservationFact.where {
                    def o1 = ObservationFact
                    startDate < ObservationFact.where { b ->
                        def o2 = ObservationFact
                        subquery.criteria.each { b.add(it) }
                        o1.patient == o2.patient
                    }.min(startTimeField.fieldName)
                }
            case Operator.AFTER:
                return ObservationFact.where {
                    def o1 = ObservationFact
                    startDate > ObservationFact.where { b ->
                        ...
                        def o2 = ObservationFact
                        o1.patient == o2.patient
                    }.max(startTimeField.fieldName)
                }
            case Operator.EXISTS:
                return ObservationFact.where { b1 ->
                    def o1 = ObservationFact
                    b1.exists ObservationFact.where { b2 ->
                        subquery.criteria.each { b2.add(it) }
                        def o2 = ObservationFact
                        o1.patient == o2.patient
                    }
                }.id()
    ...
    DetachedCriteria build(Constraint constraint) {
    ...
    DetachedCriteria build(ObservationQuery query) {
        ...
        def criteria = build(query.constraint).build {
            aliases.each { property, alias ->
                if (property != 'observation_fact') {
                    createAlias(property, alias)
                }
            }
            'in'("${trialVisitAlias}.study", getStudies())
        }
        switch (query.queryType) {
            case QueryType.MAX:
                assert query.select?.size() == 1
                return criteria.max(query.select[0])
            case QueryType.MIN:
                assert query.select?.size() == 1
                return criteria.min(query.select[0])
    ...
    DetachedCriteria build(PatientQuery query) {
        throw new NotImplementedException()
    }
