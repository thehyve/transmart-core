CREATE OR REPLACE PROCEDURE "TM_CZ"."I2B2_CREATE_GENOTYPE_TREE" 
(
  trial_id         VARCHAR2
 ,top_node        varchar2
...
AS
--  HZ @20151208  modified from i2b2_process_SNP_data
-- procedure to create clinical tree hierarchy for genotyping data without loading genotyping SNP data
...
BEGIN
    ...
    select count(distinct source_cd) into sCount from lt_src_mrna_subj_samp_map;
    if sCount>1 then 
        raise multiple_sourcecd;
    end if;
    select distinct source_cd into sourceCd from lt_src_mrna_subj_samp_map;
   -- sourceCd := 'STD';
  ...
  END IF;
    ...
    stepCt := 0;
    stepCt := stepCt + 1;
    cz_write_audit(jobId,databaseName,procedureName,'Starting i2b2_create_genotype_tree',0,stepCt,'Done');
...
/*
    select count(*) into pCount
    from BIOMART.BIO_ASSAY_GENO_PLATFORM_PROBE a, biomart.BIO_ASSAY_PLATFORM b
    where a.bio_assay_platform_id=b.bio_assay_platform_id and PLATFORM_ACCESSION in (select distinct m.platform from lt_src_mrna_subj_samp_map m);

    if PCOUNT = 0 then
        RAISE UNMAPPED_platform;
    end if;
*/
    --    check if platform exists in bio_assay_platform .  If not, abort run.
    select count(*) into pCount
    from biomart.bio_assay_platform
    where platform_accession in (select distinct m.platform from lt_src_mrna_subj_samp_map m);
    ...
    if PCOUNT=0 then
        insert into deapp.de_gpl_info 
          ...
          select 
             ...
             SYSDATE,
             platform_type
          from biomart.bio_assay_platform where platform_accession in 
          (select distinct m.platform from lt_src_mrna_subj_samp_map m);
    end if;
       
    pCount := SQL%ROWCOUNT;

    stepCt := stepCt + 1;
    cz_write_audit(jobId,databaseName,procedureName,'Insert platform to deapp.de_gpl_info',pCount,stepCt,'Done');
    commit;
...
*/
    ...
    from (select distinct 'Unknown' as sex_cd,
         ...
         from lt_src_mrna_subj_samp_map s
             ,biomart.bio_assay_platform g
         where s.subject_id is not null
           and s.trial_name = TrialID
           and s.source_cd = sourceCD
           and s.platform = g.platform_accession
           and upper(g.platform_type) = 'SNP'
           and not exists
...
--    from wt_subject_mrna_data
    ...
    where a.trial_name = TrialID
      and nvl(a.platform,'GPL570') = g.platform_accession
      and a.source_cd = sourceCD
      and a.platform = g.platform_accession
      and upper(g.platform_type) = 'SNP'
      ;
...
--    Insert records for patients and samples into observation_fact
  ...
  --Reload Security: Inserts one record for every I2B2 record into the security table
    ...
    i2b2_load_security_bystudy(TrialID, jobID);
...
*/
    stepCt := stepCt + 1;
    cz_write_audit(jobId,databaseName,procedureName,'End I2B2_CREATE_GENOTYPE_TREE',0,stepCt,'Done');
    ...
    when unmapped_platform then
        cz_write_audit(jobId,databasename,procedurename,'Platform not found in bio_assay_platform or bio_assay_geno_platform_probe',1,stepCt,'ERROR');
        CZ_ERROR_HANDLER(JOBID,PROCEDURENAME);
        cz_end_audit (jobId,'FAIL');
    ...
    when multiple_sourcecd then
        cz_write_audit(jobId,databasename,procedurename,'Multiple source_cd for sample_cd in lt_src_mrna_subj_samp_map',1,stepCt,'ERROR');
        CZ_ERROR_HANDLER(JOBID,PROCEDURENAME);
        cz_end_audit (jobId,'FAIL');
