...
BEGIN
	...
	DataType:=upper(data_type);
     ...
     --If Job ID does not exist, then this is a single procedure run and we need to create it
        ...
        stepCt := 0;
	stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'Starting I2B2_MIRNA_INC_SUB_ZSCORE',0,stepCt,'Done');
  ...
  if  DataType='R'
    then 
        for UpdateZscore in zscore_params_qpcr
          loop
            update de_subject_mirna_data d 
	...
	cz_write_audit(jobId,databaseName,procedureName,'update zscore calc for this trial',0,stepCt,'Done');
	  ...
	  cz_write_audit(jobId,databaseName,procedureName,'Normalize the zscore calc value',0,stepCt,'Done');
          ...
          stepCt := stepCt + 1;
	  cz_write_audit(jobId,databaseName,procedureName,'END of I2B2_MIRNA_INC_SUB_ZSCORE',0,stepCt,'Done');
          commit; 
