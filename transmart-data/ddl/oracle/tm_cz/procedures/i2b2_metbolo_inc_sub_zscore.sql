--
-- Type: PROCEDURE; Owner: TM_CZ; Name: I2B2_METBOLO_INC_SUB_ZSCORE
--
  CREATE OR REPLACE PROCEDURE "TM_CZ"."I2B2_METBOLO_INC_SUB_ZSCORE" 
( trial_id IN VARCHAR
 ,currentJobID 	NUMBER := null
   ...
   --Audit variables
    ...
    SELECT AVG(d.log_intensity)mean_value,median(d.log_intensity)median_value,STDDEV(d.log_intensity)stddev_value,
    d.metabolite_annotation_id
    FROM DE_SUBJECT_METABOLOMICS_DATA d
    WHERE d.trial_name = trial_id
    GROUP BY d.trial_name, d.metabolite_annotation_id;
        ...
        -- call the cursor and update the z-score value for incremental data;
                  for UpdateZscore in zscore_params
                    loop
                        ...
                        update DE_SUBJECT_METABOLOMICS_DATA d 
                                set d.zscore=(CASE WHEN UpdateZscore.stddev_value=0 THEN 0 ELSE (d.log_intensity - UpdateZscore.median_value ) / UpdateZscore.stddev_value END)
                                              ...
                                              and d.metabolite_annotation_id=UpdateZscore.metabolite_annotation_id;
	...
	cz_write_audit(jobId,databaseName,procedureName,'update zscore calc for this trial',0,stepCt,'Done');
        commit;
          ...
          --Normalize the  zscore value when greater than 2.5 and lesser than -2.5
              ...
              update DE_SUBJECT_METABOLOMICS_DATA 
	  ...
	  cz_write_audit(jobId,databaseName,procedureName,'Normalize the zscore calc value',0,stepCt,'Done');
          ...
          stepCt := stepCt + 1;
	  cz_write_audit(jobId,databaseName,procedureName,'END of I2B2_METBOLO_INC_SUB_ZSCORE',0,stepCt,'Done');
          commit; 
