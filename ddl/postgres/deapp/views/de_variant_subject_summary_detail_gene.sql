...
CREATE VIEW de_variant_subject_summary_detail_gene AS
 SELECT summary.variant_subject_summary_id,
   ...
   JOIN de_variant_subject_detail detail ON (((((detail.dataset_id)::text = (summary.dataset_id)::text) AND ((detail.chr)::text = (summary.chr)::text)) AND (detail.pos = summary.pos))))
