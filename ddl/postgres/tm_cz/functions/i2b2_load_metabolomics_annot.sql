--
-- Name: i2b2_load_metabolomics_annot(bigint); Type: FUNCTION; Schema: tm_cz; Owner: -
--
CREATE OR REPLACE FUNCTION i2b2_load_metabolomics_annot (
...
BEGIN
	...
	PERFORM 
        deapp.nextval('metabolomics_annot_id')
        ,d.gpl_id
	...
	PERFORM 
        deapp.nextval('metabolite_sup_pth_id')
        ,d.gpl_id
	,d.super_pathway
	...
	cz_write_audit(jobId,databaseName,procedureName,'Load annotation data into DEAPP de_metabolite_super_pathways',SQL%ROWCOUNT,stepCt,'Done');
        ...
        PERFORM 
        deapp.nextval('metabolite_sub_pth_id')
        ,d.gpl_id
	,trim(d.sub_pathway)
        ,sp.id
	...
	PERFORM b.metabolite_id,a.ID from deapp.de_metabolite_sub_pathways a,
...
/*	update deapp.de_peptide_annotation t
	...
	*/
	...
	where t.gpl_id = gplId
	  and coalesce(t.biomarker_id::text, '') = ''
	  and (t.biochemical_name IS NOT NULL AND t.biochemical_name::text <> '')
	  and exists
		 (select 1 from biomart.mirna_bio_marker x
	...
	(feature_group_name
	,feature_group_type)
	PERFORM distinct t.biochemical_name, 'METABOLOMIC' --ask
	...
	,bio_marker_id)
	PERFORM distinct fg.bio_assay_feature_group_id
	...
	where ((t.hmdb_id IS NOT NULL AND t.hmdb_id::text <> ''))
