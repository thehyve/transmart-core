--
-- Name: i2b2_load_mirna_annot_deapp(bigint); Type: FUNCTION; Schema: tm_cz; Owner: -
--
CREATE FUNCTION i2b2_load_mirna_annot_deapp(currentjobid bigint DEFAULT NULL::bigint) RETURNS void
    LANGUAGE plpgsql
    AS $_$
DECLARE
...
*/
	...
	where t.id_ref = idREF
	  and coalesce(t.mirna_id::text, '') = '' 
	  and (t.mirna_symbol IS NOT NULL AND t.mirna_symbol::text <> '')
	  and exists
	...
	set mirna_symbol=(select min(b.bio_marker_name) as mirna_symbol
				 ...
				 where to_char(t.mirna_id) = b.primary_external_id
				   ...
				   and upper(b.bio_marker_type) = 'MIRNA')
	where t.id_ref = idREF
	  and coalesce(t.mirna_symbol::text, '') = ''
	  and (t.mirna_id IS NOT NULL AND t.mirna_id::text <> '')
	  and exists
		 (select 1 from biomart.mirna_bio_marker x
	...
	(feature_group_name
	,feature_group_type)
	PERFORM distinct t.id_ref, 'PROBESET'
	...
	where ( --t.mirna_symbol is not null or 
               (t.mirna_id IS NOT NULL AND t.mirna_id::text <> ''))
