...
/*	update deapp.de_peptide_annotation t
	...
	*/
	...
	where t.gpl_id = gplId
	  ...
	  and (t.uniprot_id IS NOT NULL AND t.uniprot_id::text <> '')
	  and exists
		 (select 1 from biomart.mirna_bio_marker x
	...
	(feature_group_name
	,feature_group_type)
	PERFORM distinct t.peptide, 'PEPTIDE' --ask
	...
	where ( (t.biomarker_id IS NOT NULL AND t.biomarker_id::text <> '') or 
               (t.uniprot_id IS NOT NULL AND t.uniprot_id::text <> ''))
