...
--	If not, raise exception
	...
	if runType = 'L' then
		...
		if stgTrial != TrialId then
			stepCt := stepCt + 1;
			cz_write_audit(jobId,databaseName,procedureName,'TrialId not the same as trial in wt_subject_rna_probeset - procedure exiting'
,SQL%ROWCOUNT,stepCt,'Done');
...
--	truncate tmp tables
	...
	if idxExists = 1 then
		EXECUTE('drop index tm_wz.WT_SUBJECT_RNA_LOGS_I1');		
	end if;
	...
	if idxExists = 1 then
		EXECUTE('drop index tm_wz.WT_SUBJECT_RNA_CALCS_I1');
	end if;
...
--		end if;
	...
	EXECUTE('create index tm_wz.WT_SUBJECT_RNA_LOGS_I1 on tm_wz.wt_subject_rna_logs (trial_name, probeset_id) nologging  tablespace "INDX"');
	stepCt := stepCt + 1;
...
--	calculate mean_intensity, median_intensity, and stddev_intensity per experiment, probe
	...
	EXECUTE('create index tm_wz.WT_SUBJECT_RNA_CALCS_I1 on tm_wz.wt_subject_rna_calcs (trial_name, probeset_id) nologging tablespace "INDX"');
	stepCt := stepCt + 1;
