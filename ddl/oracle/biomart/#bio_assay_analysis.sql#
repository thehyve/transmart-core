...
"BIO_ASSAY_ANALYSIS_ID" NUMBER(18,0) NOT NULL ENABLE, --postgres NOT NULL as primary key
...
"ANALYSIS_UPDATE_DATE" DATE, 	--postgres
"LSMEAN_CUTOFF" NUMBER(9,2), 	--postgres
...
--
  CREATE OR REPLACE EDITIONABLE TRIGGER "BIOMART"."TRG_BIO_ASSAY_ANALYSIS_ID" before insert on "BIO_ASSAY_ANALYSIS"    for each row begin     if inserting then       if :NEW."BIO_ASSAY_ANALYSIS_ID" is null then          select SEQ_BIO_DATA_ID.nextval into :NEW."BIO_ASSAY_ANALYSIS_ID" from dual;       end if;    end if; end;
...
--
  CREATE OR REPLACE EDITIONABLE TRIGGER "BIOMART"."TRG_BIO_ASSAY_ANALYSIS_UID" after insert on "BIO_ASSAY_ANALYSIS"    
for each row
DECLARE
  rec_count NUMBER;
BEGIN
