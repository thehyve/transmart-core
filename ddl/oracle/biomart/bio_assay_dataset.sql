...
--
  CREATE OR REPLACE EDITIONABLE TRIGGER "BIOMART"."TRG_BIO_ASSAY_DATASET_ID" before insert on "BIO_ASSAY_DATASET"    for each row begin     if inserting then       if :NEW."BIO_ASSAY_DATASET_ID" is null then          select SEQ_BIO_DATA_ID.nextval into :NEW."BIO_ASSAY_DATASET_ID" from dual;       end if;    end if; end;
