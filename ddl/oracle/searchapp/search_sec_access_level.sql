...
--
 CREATE TABLE "SEARCHAPP"."SEARCH_SEC_ACCESS_LEVEL" 
  (	"SEARCH_SEC_ACCESS_LEVEL_ID" NUMBER(18,0) NOT NULL ENABLE, --postgres NOT NULL
...
--
  CREATE OR REPLACE EDITIONABLE TRIGGER "SEARCHAPP"."TRG_SEARCH_SEC_ACC_LEVEL_ID" before insert on "SEARCH_SEC_ACCESS_LEVEL"    for each row begin     if inserting then       if :NEW."SEARCH_SEC_ACCESS_LEVEL_ID" is null then          select SEQ_SEARCH_DATA_ID.nextval into :NEW."SEARCH_SEC_ACCESS_LEVEL_ID" from dual;       end if;    end if; end;
